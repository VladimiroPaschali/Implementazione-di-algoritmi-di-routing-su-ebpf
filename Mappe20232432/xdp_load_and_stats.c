...
void mappa(int fds[]){
	...
	int numfd[]={20,23,24,32};
	//legge i file nelle 3 mappe
	for(int i=0; i<=3;i++){
		__u64 count=0;

		FILE * fp;
		char stringa[50];
		sprintf(stringa,"mappe/%d.txt",numfd[i]);
		fp=fopen(stringa,"r");
		char * line = NULL;
	    	size_t len = 0;
	    	ssize_t read;
		...
		while ((read = getline(&line, &len, fp)) != -1) {
			...
			//carica gli ip convertiti nelle mappe da 24 a 32
		  	assert(bpf_map_update_elem(fds[i],&ip,&pass,BPF_ANY)==0);
			count++;
			counttot++;
...
int main(int argc, char **argv)
{
	...
	int fds[]={map_lpm20_fd,map_lpm23_fd,map_lpm24_fd,map_lpm32_fd};
